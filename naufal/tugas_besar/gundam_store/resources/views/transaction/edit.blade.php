    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <br />
                    <form action="/transactions/{{ $id }}" method="POST" @submit="submitForm($event, {{ $id }})" class="form-label-left input_mask">
                        @csrf
                        @method('PUT')
                        <div class="form-group row">
                            ...
                            <div class="col-md-9 col-sm-9 ">
                                <select name="member_id" class="form-control @error('member_id') is-invalid @enderror" id="member_id" required readonly>
                                    <option value="{{ $transactions[0]->member_id }}">{{ $transactions[0]->name }}</option>
                                </select>
                                @error('member_id')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9 col-sm-9 ">
                                <input name="purchase_date" class="date-picker form-control @error('purchase_date') is-invalid @enderror" value="{{ $transactions[0]->purchase_date }}" type="date" readonly required="required" >
                                @error('purchase_date')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9 col-sm-9 ">
                                <input name="repayment_date" class="date-picker form-control @error('repayment_date') is-invalid @enderror" value="{{ $transactions[0]->repayment_date }}" type="date" readonly required="required">
                        ...
                        <div class="form-group row">
                            <div class="col-md-6 form-group">
                                ...
                                <select class="form-control @error('gundam_product_id') is-invalid @enderror"  name="gundam_product_id[]" id="product" disabled>
                                    <option value="0">-- Select Product --</option>
                                    @foreach($products as $keyProd => $product)
                                    <option value="{{ $product->id }}" {{ $product->id == $transaction->gundam_product_id ? 'selected' : '' }}>{{ $product->product_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <input class="form-control" value="{{ $transaction->price }}" type="number" id="price" readonly>
                            </div>

                            <div class="col-md-2 form-group">
                                <label for="quantity">Quantity</label>
                                <input name="purchase_qty[]" value="{{ $transaction->purchase_qty }}" class="form-control @error('purchase_qty') is-invalid @enderror" type="number" id="quantity" readonly>
                                @error('purchase_qty')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group-row">
                            ...
                            <div class="col-md-auto col-sm-auto ">
                                <input name="status" type="radio" class="flat @error('status') is-invalid @enderror" value="1" {{ $transactions[0]->status == '1' ? 'checked' : ''}} > Paid
                                @error('status')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-auto col-sm-auto ">
                                <input name="status" type="radio" class="flat @error('status') is-invalid @enderror" value="0" {{ $transactions[0]->status != '1' ? 'checked' : '' }}> Not paid
                                @error('status')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row mt-2">
                            ...
                            <div class="col-md-9 col-sm-9 ">
                                <input name="total_price" type="number" class="form-control @error('total_price') is-invalid @enderror" value="{{ $transaction->total_price }}" readonly >
                                @error('total_price')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
